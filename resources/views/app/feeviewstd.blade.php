            ...
            <div class="col-md-12">
              <table class="table">
                ...
                <tbody>
                  ...
                  <tr>
                    <td></td>
                    <td>Total Payable: <strong><i class="blue">Rs.&nbsp;{{$totals->payTotal}}</i></strong> </td>
                    <td>Total Paid: <strong><i class="blue">Rs.&nbsp;{{$totals->paiTotal}}</i></strong> </td>
                    <td>Total Due: <strong><i class="blue">Rs.&nbsp;{{$totals->dueAmount}}</i></strong> </td>
                    <td></td>

                    <td>
                    <br>
                      <select id="billSelect" name="billSelect" class="form-control" required="true">
                        <option value="">-- Select Bill No --</option>
                        @foreach($billNos as $billNo)
                          <option value="{{ $billNo->billNo }}">{{ $billNo->billNo }}</option>
                        @endforeach
                      </select>
                      <br>
                      <button onclick="printBIlling()" class='btn btn-info float-right'><i class="glyphicon glyphicon-print icon-red"></i> Print</button>
                      <!-- <a style="float: right;" title='Print' id="btnPrint" class='btn btn-info float-right' target='_blank' href='{{url("/fees/report/std")}}/{{$student->regiNo}}'> <i class="glyphicon glyphicon-print icon-red"></i> Print</a> -->
                    </td>
      ...
      <div id="billDetails" class="modal fade">
        <div class="modal-dialog">
          ...
          $( document ).ready(function() {
            ...
            var studentRegNo  = '{{ $student->regiNo }}'
            

            $("#billSelect").on('change', function(){
               $("#btnPrint").show()
               reportLink = '/fees/report/std/' + studentRegNo + '/' + $("#billSelect").val()
            })
