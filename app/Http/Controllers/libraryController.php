...
class libraryController extends Controller {
	...
	{
		$classes = array('All'=> ClassModel::pluck('name','code'));
	...
	*/
	...
	public function getissueBook()
	{
		...
		$students = Student::select(DB::raw("CONCAT(firstName,' ',middleName,' ',lastName,'[',regiNo,']') as name"),'regiNo')->get();
		// $books = ['Select Book' => AddBook::select(DB::raw("CONCAT(title,'[',author,']') as name,code"))->pluck('name','code')];
		$books = AddBook::select(DB::raw("CONCAT(title,'[',author,']') as name,code"))->get();
	...
	public function postissueBook()
	{
		...
		// $issuebook->code = Input::get('bookCode');
